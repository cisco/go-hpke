...
func (priv ecdhPrivateKey) PublicKey() DHPublicKey {
...
func (s ecdhScheme) Generate(rand io.Reader) (DHPrivateKey, DHPublicKey, error) {
...
func (s ecdhScheme) Derive(priv DHPrivateKey, pub DHPublicKey) ([]byte, error) {
...
func (priv x25519PrivateKey) PublicKey() DHPublicKey {
	pub := &x25519PublicKey{}
...
func (s x25519Scheme) Generate(rand io.Reader) (DHPrivateKey, DHPublicKey, error) {
...
func (s x25519Scheme) Derive(priv DHPrivateKey, pub DHPublicKey) ([]byte, error) {
...
func (priv x448PrivateKey) PublicKey() DHPublicKey {
	pub := &x448PublicKey{}
...
func (s x448Scheme) Generate(rand io.Reader) (DHPrivateKey, DHPublicKey, error) {
	priv := &x448PrivateKey{}
	_, err := rand.Read(priv.val[:])
	if err != nil {
		return nil, nil, err
...
func (s x448Scheme) Derive(priv DHPrivateKey, pub DHPublicKey) ([]byte, error) {
	...
	if !ok {
		return nil, fmt.Errorf("Private key not suitable for X448")
	}
...
var ciphersuites = map[byte]Ciphersuite{
	...
	P256_HKDF_SHA256_AESGCM128: {
		...
		DH:   ecdhScheme{curve: elliptic.P256()},
		KDF:  hkdfScheme{hash: crypto.SHA256},
		AEAD: aesgcmScheme{keySize: 16},
	},
	...
	P521_HKDF_SHA512_AESGCM256: {
		...
		DH:   ecdhScheme{curve: elliptic.P521()},
		KDF:  hkdfScheme{hash: crypto.SHA512},
		AEAD: aesgcmScheme{keySize: 32},
	},
